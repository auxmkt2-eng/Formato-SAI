...
async function importarExcelASupabase(file, marca, sede) {
  ...
  if (registros.length === 0) {
    return { insertados: 0, errores: errores.length ? errores : ["No se encontró ninguna fila con datos de paciente."] };
  }
  ...
  return { insertados: registros.length, errores };
}
  ...
  input.addEventListener("change", async () => {
    ...
    try {
      ...
      if (resultado.insertados > 0) {
        showToast(`${resultado.insertados} registro(s) importado(s)${resultado.errores.length ? ` · ${resultado.errores.length} fila(s) con errores (ver consola)` : ""}`, "success");
